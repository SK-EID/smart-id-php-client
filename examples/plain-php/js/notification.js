...
async function checkStatus() {
    ...
    if (data.state === 'COMPLETE') {
        ...
        if (data.endResult === 'OK' && data.user) {
            ...
            statusEl.innerHTML = `<span style="color: #dc2626;">Validation failed: ${data.error || 'Unknown error'}</span>${tryAgainBtn}`;
        } else {
            statusEl.innerHTML = `<span style="color: #dc2626;">Authentication failed: ${data.endResult || 'Unknown error'}</span>${tryAgainBtn}`;
        }
    }
}
